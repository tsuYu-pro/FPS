...
FString GetNodeTitleString(UPCGNode* Node)
{
    ...
    // UE 5.4 needs the title type: UPCGNode::GetNodeTitle(EPCGNodeTitleType).
    // The zero-argument overload never existed before 5.5, so this branch could
    // not compile on 5.4 (caught by a UE 5.4.4 editor build).
...
UPCGNode* FindPCGNode(UPCGGraph* Graph, const FString& NodeId)
{
    ...
    for (UPCGNode* Node : Nodes)
    {
        ...
        // UE 5.4: GetNodeTitle requires the title type (see the note above).
